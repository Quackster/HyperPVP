...
public class PinCommand implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		try {
			...
			if (HyperPVP.getStorage().readString("SELECT password FROM users WHERE uuid = '" + player.getUniqueId().toString() + "'").length() > 0) {
				sender.sendMessage(ChatColor.RED + "You have already made your account.");
				return true;
			}
			
			String pin = HyperPVP.getStorage().readString("SELECT pin FROM users WHERE uuid = '" + player.getUniqueId().toString() + "'"); 
