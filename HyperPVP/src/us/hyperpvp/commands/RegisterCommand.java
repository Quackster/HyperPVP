...
public class RegisterCommand implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command command, String label, final String[] args) {
		...
		try {
			row = HyperPVP.getStorage().readRow("SELECT id,password,salt FROM users WHERE uuid = '" + player.getUniqueId().toString() + "'");
			...
			if (password.length() == 0 || salt.length() == 0) {
				...
				HyperPVP.getStorage().executeQuery("UPDATE users SET pin = '" + pin + "' WHERE id = '" + id + "'");
